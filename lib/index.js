...
function init(_a) {
    ...
    function create(info) {
        ...
        if (options.customRenderer) {
            if (fs_1.existsSync(path_1.default.resolve(directory, options.customRenderer))) {
                options.customRenderer = path_1.default.resolve(directory, options.customRenderer);
            }
            else if (fs_1.existsSync(require.resolve(options.customRenderer))) {
        ...
        var _createLanguageServiceSourceFile = typeSystem.createLanguageServiceSourceFile;
        typeSystem.createLanguageServiceSourceFile = function (fileName, scriptSnapshot) {
            var rest = [];
            for (var _i = 2; _i < arguments.length; _i++) {
                rest[_i - 2] = arguments[_i];
            }
            if (isCSS(fileName)) {
                scriptSnapshot = getDtsSnapshot_1.getDtsSnapshot(typeSystem, processor, fileName, scriptSnapshot, options, logger, compilerOptions);
            }
        ...
        var _updateLanguageServiceSourceFile = typeSystem.updateLanguageServiceSourceFile;
        typeSystem.updateLanguageServiceSourceFile = function (sourceFile, scriptSnapshot) {
            var rest = [];
            for (var _i = 2; _i < arguments.length; _i++) {
                rest[_i - 2] = arguments[_i];
            }
            if (isCSS(sourceFile.fileName)) {
                scriptSnapshot = getDtsSnapshot_1.getDtsSnapshot(typeSystem, processor, sourceFile.fileName, scriptSnapshot, options, logger, compilerOptions);
            }
                    ...
                    try {
                        if (isRelativeCSS(moduleName)) {
                            ...
                            var cssModulePath = normalizedLocations.find(function (location) {
                                return fs_1.existsSync(location);
                            });
                            if (cssModulePath) {
                                return {
                                    extension: '.d.ts',
